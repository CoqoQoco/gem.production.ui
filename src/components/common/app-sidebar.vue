    ...
    <transition name="slide">
      <aside v-if="isOpen" class="app-sidebar">
        ...
        <nav class="sidebar-nav">
          ...
          <div v-if="hasAccessibleChildren(['StockSummary', 'GoodsReceipt', 'UpdateProduct', 'MassUpdateGoldPrice', 'MassUploadImage', 'MassTransferProduct', 'StockList', 'StockIn', 'StockOut', 'StockTransfer'])" class="nav-group">
            ...
            <transition name="submenu-slide">
              <div v-if="openSubmenu === 'inventory'" class="submenu">
                ...
                <router-link
                  v-if="canAccess('MassTransferProduct')"
                  to="/inventory/mass-transfer-product"
                  class="submenu-item"
                  active-class="active"
                  @click="closeSidebar"
                >
                  <i class="pi pi-circle-fill"></i>
                  <span>{{ t('dashboard.menu.massTransferProduct') || 'โอนย้ายสินค้า' }}</span>
                </router-link>
                <router-link
